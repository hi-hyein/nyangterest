...
#### 도로 원점이라 아래 신나게(?) 썼던 글은 지웠다...

* 에효.. 다시 확인해보니 프론트 listStore.js에 kind에 빈값("")만 아니면 어떠한 값을 넣어도 url은 전체 고양이품종이 나오는걸 확인하게 되었다.

* 그럼 결국 백엔드에서 전체고양이를 불러오려면 kind=&을 하거나 아예 요청변수 kind를 넣지 않아야 한다는 것인데 그럼 품종 선택전 url과  선택후 url을 달리 해야 하는거 아닌가 싶다. 
